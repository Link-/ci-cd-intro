...
# changes.
#
resource "random_id" "server_PR_13" {
  keepers = {
...
resource "aws_instance" "staging_cicd_demo_PR_13" {
  # Read the AMI id "through" the random_id resource to ensure that
  # both will change together.
  ami                    = random_id.server_PR_13.keepers.ami_id
  instance_type          = "t2.micro"
  vpc_security_group_ids = ["sg-0d2411db69a112a30"]
  key_name               = aws_key_pair.staging_key.key_name

  tags = {
    "Name" = "staging_cicd_demo-PR_13"
  }
}

output "staging_dns_PR_13" {
  value = aws_instance.staging_cicd_demo_PR_13.public_dns
}
